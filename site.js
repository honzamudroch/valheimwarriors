...
style.textContent = `
body{position:relative;background:#0b1424!important;background-image:
  radial-gradient(ellipse 60% 40% at 8% 6%,rgba(255,214,150,.42),transparent 60%),
...
.achpop .note.warn{color:var(--ink-2);border-left:3px solid var(--ember);padding-left:8px;line-height:1.4}
.achpop .note.warn b{color:var(--ember)}
...
.hero2{display:grid;grid-template-columns:minmax(300px,1fr) minmax(320px,.95fr);gap:28px;align-items:start;border:2px solid var(--line);background:linear-gradient(180deg,rgba(42,34,24,.93),rgba(31,25,18,.94) 40%,rgba(22,18,9,.96));backdrop-filter:blur(2px);box-shadow:inset 0 0 0 1px var(--panel-3),inset 0 0 0 3px var(--line-2),0 10px 30px var(--shadow);padding:30px 32px 0 32px;position:relative;overflow:hidden}
...
.vwmodal .guide li{margin:0 0 10px}
.vwmodal .red{color:#f0a090}
.vwmodal .red b{color:#ffb8a8}
...
const helpHTML = en => `<h3>${en ? 'Help' : 'Nápověda'}</h3>
      ...
      <details><summary>${en ? 'The Sync app says the server has the same or a newer save' : 'Sync appka hlásí stejné nebo novější uložení na serveru'}</summary><p>${en ? 'You uploaded that very file in the browser a moment ago, so the app has nothing newer to offer. Just play: after the next game save (logout or ~20 minutes) the app uploads the newer file and takes the character over by itself. Nothing to configure.' : 'Ten samý soubor jsi před chvílí nahrál v prohlížeči, appka nemá nic novějšího. Stačí hrát: po dalším uložení hry (odhlášení nebo ~20 minut) appka nahraje novější soubor a postavu převezme sama. Nic nenastavuješ.'}</p></details>
...
function openModal(kind, data){
  ...
  if(kind === 'char' && data){
    ...
    body = `<p class="lead">${en ? 'A small Windows program that sits in the tray, watches your Steam character folder and uploads your character to your Valhalla after every game save. You never drag the file again and your party always sees fresh numbers.' : 'Malý program pro Windows, který sedí v liště u hodin, hlídá složku s postavami ve Steamu a po každém uložení hry nahraje tvoji postavu do Valhaly. Soubor už nikdy nepřetahuješ a parta vidí vždy čerstvá čísla.'}</p>
      <ol class="guide">
      <li><b>${en ? 'Download and run' : 'Stáhni a spusť'} <code>ValheimWarriorsSync.exe</code>.</b><br><span class="red">${en ? 'Windows will warn "unknown publisher" (the app is an unsigned beta). Click <b>More info</b>, then <b>Run anyway</b>. On a PC with Smart App Control turned on it will not start at all; use the web page there.' : 'Windows zahlásí "neznámý vydavatel" (appka je nepodepsaná beta). Klikni <b>Další informace</b> a pak <b>Přesto spustit</b>. Na PC se zapnutým Smart App Control se nespustí vůbec; tam nahrávej přes web.'}</span></li>
      <li><b>${en ? 'Set it up once:' : 'Nastav jednou:'}</b> ${en ? 'paste your Valhalla link (or press Find my Valhallas), tick your character, optionally Start with Windows, press Save.' : 'vlož odkaz své Valhaly (nebo dej Najít moje Valhaly), zaškrtni svoji postavu, případně Spouštět při startu Windows, dej Uložit.'}</li>
      <li><b>${en ? 'Done.' : 'Hotovo.'}</b> ${en ? 'It sits in the tray and uploads after every game save. If you uploaded the same file on the web a moment ago, it waits for the next save and then takes over. The sheet shows a green "Sync app" badge.' : 'Sedí v liště u hodin a nahrává po každém uložení hry. Pokud jsi ten samý soubor před chvílí nahrál na webu, počká na další uložení a pak postavu převezme. Na listu je zelený štítek "Sync appka".'}</li></ol>
      <div class="mrow"><a class="sitebtn" href="/download/ValheimWarriorsSync.exe" download style="text-decoration:none">${en ? 'Download for Windows' : 'Stáhnout pro Windows'} <small>· v0.1.5 · 30 MB</small></a></div>
      <div class="mrow small"><details class="sha"><summary>${en ? 'Verify the download' : 'Ověření staženého souboru'}</summary>${en ? 'Unsigned apps cannot prove who made them, so here is the fingerprint of the file I published. In PowerShell run <code>Get-FileHash ValheimWarriorsSync.exe</code>; the result must be' : 'Nepodepsaná appka nemůže prokázat, kdo ji vydal, proto je tady otisk zveřejněného souboru. V PowerShellu spusť <code>Get-FileHash ValheimWarriorsSync.exe</code>; výsledek musí být'} <code>7cc4ac29ba292763efffa0b3c34eaccf98c9bdb0325b67d18cab27b6e5b940ec</code>. ${en ? 'If it differs, do not run the file.' : 'Když se liší, soubor nespouštěj.'}</details></div>
...
function showInvite(withAdmin){
  ...
  box.innerHTML = `<h4>${en ? 'Invite the party' : 'Pozvi partu'}<span>${esc(SERVER.name)}</span></h4>
    ...
    ${withAdmin && ADMIN ? `<div class="note warn" style="margin:10px 0 4px"><b>${en ? 'Save the admin link now' : 'Admin odkaz si teď ulož'}</b> ${en ? '(notes, a message to yourself). It is the only way to manage this Valhalla: remove characters, open it as admin on another device. There are no accounts, so a lost link cannot be recovered automatically. Never share it with the party.' : '(poznámky, zpráva sám sobě). Je to jediný způsob, jak Valhalu spravovat: mazat postavy, otevřít správu na jiném zařízení. Účty nemáme, ztracený odkaz nejde obnovit automaticky. Partě ho neposílej.'}</div><div class="a" style="grid-template-columns:1fr auto"><code style="font-size:12px;word-break:break-all">${esc(link + '#admin=' + ADMIN)}</code><button class="sitebtn" data-copy="${esc(link + '#admin=' + ADMIN)}">${en ? 'Copy' : 'Kopírovat'}</button></div>` : ''}
...
window.SITE_UPLOAD = async d => {
  ...
  try{ t = await rpc('vw_upsert_character', {p_slug: slug, p_data: d, p_saved_at: d.meta.saved || null, p_token: tok}); }
    ...
    const k = prompt(EN() ? `${d.name}: the Valhalla already has the same or a newer save of this character from another device (a newer save always wins). Play and save, then upload again. Or paste the character key from that device:` : `${d.name}: ve Valhale už je stejné nebo novější uložení této postavy z jiného zařízení (novější uložení vždy vyhrává). Zahraj si, ulož a nahraj znovu. Nebo vlož klíč postavy z toho zařízení:`);
